...
model_path = os.path.join(current_path, 'models/model.pt')
# Pen parameters
pen_color = (255, 0, 0)
eraser_size = 80
pen_size = 10
...
while True:
    ...
    if not results.multi_hand_landmarks:
        was_drawing_last_frame = False
        cv2.putText(frame, 'No hand in frame', (w - 300, h - 50),
        ...
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            cv2.putText(frame, f"Mode : {action}", (w - 300,
                        h - 50), font, fontScale, fontColor, lineType)
            ...
            if action == 'Draw':
                ...
                if was_drawing_last_frame:
                    ...
                    num_step_points = int(
                        int(distance) // intermediate_step_gap) - 1
            ...
            if action == 'Erase':
                eraser_mid = [
                    int(
                        hand_landmarks.landmark[mpHands.HandLandmark.MIDDLE_FINGER_MCP].x * w),
                    int(
                        hand_landmarks.landmark[mpHands.HandLandmark.MIDDLE_FINGER_MCP].y * h)
                ]

                bottom_right = (
                    eraser_mid[0] + eraser_size, eraser_mid[1] + eraser_size)
                top_left = (eraser_mid[0] - eraser_size,
                            eraser_mid[1] - eraser_size)
    ...
    ptime = ctime
    cv2.putText(frame, f'FPS : {fps}', (w - 300, h - 20),
                font, fontScale, fontColor, lineType)

    cv2.imshow('output', frame)
    # contol_image = img[:80, :]
    img[button[0]:button[1], button[2]:button[3]] = 180
    cv2.putText(img, 'Click_to_save_img', (148, 50),
                cv2.FONT_HERSHEY_TRIPLEX, 1.0, 0, 1)
